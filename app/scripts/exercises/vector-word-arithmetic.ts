...
async function demonstrateWordArithmetic() {
  ...
  // My example 1: A brief study in power
  console.log("✊  EXAMPLE: A brief study in power");
  console.log("Formula: person - compassion + power ≈ ?");

  const [personVec, compassionVec, powerVec] = await Promise.all([
    getEmbedding("person"),
    getEmbedding("compassion"),
    getEmbedding("power"),
  ]);

  const result7 = addVectors(
    subtractVectors(personVec, compassionVec),
    powerVec,
  );
  const candidates7 = [
    "tyrant",
    ...
    "bully",
    "authoritarian",
    "cream", // obviously wrong - should have low similarity
  ];
  ...
  // My example 2: A brief study in artistic success
  console.log("🎨  EXAMPLE: A brief study in artistic success");
  console.log("Formula: artist - talent + money ≈ ?");

  const [artistVec, talentVec, moneyVec] = await Promise.all([
    getEmbedding("artist"),
    getEmbedding("talent"),
    getEmbedding("money"),
  ]);

  const result8 = addVectors(subtractVectors(artistVec, talentVec), moneyVec);
  const candidates8 = [
    "influencer",
    "fraud",
    "greedy",
    ...
    "undeserving",
    "sellout",
    "vuvuzela", // obviously wrong - should have low similarity
  ];
  ...
  // My example 3: A brief study in movie categories
  console.log("🎬  EXAMPLE: A brief study in movie productions");
  console.log("Formula: movie - budget + popular ≈ ?");

  const [movieVec, budgetVec, popularVec] = await Promise.all([
    getEmbedding("movie"),
    getEmbedding("budget"),
    getEmbedding("popular"),
  ]);

  const result9 = addVectors(subtractVectors(movieVec, budgetVec), popularVec);
  const candidates9 = [
    "arthouse",
    "indie",
    "cultfollowing",
    ...
    "fanfavourite",
    "hit",
    "concrete", // obviously wrong - should have low similarity
  ];
